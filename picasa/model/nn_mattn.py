...
class PICASANET(nn.Module):
	def __init__(self,input_dim, embedding_dim, attention_dim, latent_dim,encoder_layers,projection_layers,corruption_rate,pair_importance_weight,num_attention_layers=2):
		super(PICASANET,self).__init__()

		self.embedding = GeneEmbedor(embedding_dim,attention_dim)

		self.attention_layers = nn.ModuleList([
			ScaledDotAttention(attention_dim, input_dim, pair_importance_weight)
			for _ in range(num_attention_layers)
		])
  
		self.pooling = AttentionPooling(attention_dim * num_attention_layers)
	...
	def forward(self,x_c1,x_c2):
		...
		x_c1_att_outs, x_c1_att_ws, el_attn_c1 = [], [], 0
		x_c2_att_outs, x_c2_att_ws, el_attn_c2 = [], [], 0

		for attention_layer in self.attention_layers:
			x_c1_att_out, x_c1_att_w, el_attn_c1_layer = attention_layer(x_c1_emb, x_c2_emb, x_c2_emb)
			x_c2_att_out, x_c2_att_w, el_attn_c2_layer = attention_layer(x_c2_emb, x_c1_emb, x_c1_emb)
			
			x_c1_att_outs.append(x_c1_att_out)
			x_c1_att_ws.append(x_c1_att_w)
			el_attn_c1 += el_attn_c1_layer
			
			x_c2_att_outs.append(x_c2_att_out)
			x_c2_att_ws.append(x_c2_att_w)
			el_attn_c2 += el_attn_c2_layer

		x_c1_att_out = torch.cat(x_c1_att_outs, dim=2)
		x_c1_pool_out = self.pooling(x_c1_att_out)
		x_c1_att_w = torch.stack(x_c1_att_ws).mean(dim=0)

		x_c2_att_out = torch.cat(x_c2_att_outs, dim=2)
		x_c2_pool_out = self.pooling(x_c2_att_out)
		x_c2_att_w = torch.stack(x_c2_att_ws).mean(dim=0)
