...
wdir = pp+sample
adata = an.read_h5ad(wdir+'/results/picasa_proj_u_patient.h5ad')



####### get patient data #####################

df_pmeta = pd.read_csv(wdir+'/data/tcga_lung_clinical.csv.gz')
...
def surv_plot(tag):
    df_latent = pd.DataFrame(adata.obsm['X_'+tag], index=adata.obs_names)
    ...
    for idx, factor in enumerate(factors):
        # Compute median value for grouping
        median_value = df[factor].median()
        print(factor, median_value)
        ...
        # Plot Kaplan-Meier curves for the factor
        ...
        # Add titles, labels, and p-value to the plot
    ...
    plt.tight_layout()
    plt.savefig(wdir + '/results/picasa_proj_u_patient_survival_all_factors_' + tag + '.png')
