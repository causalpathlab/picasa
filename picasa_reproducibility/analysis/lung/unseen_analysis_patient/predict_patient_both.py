...
new_adata.obsm['common'] = df_z.values

df = pd.read_csv('data/tcga_lung_clinical.csv.gz')

import scanpy as sc
import matplotlib.pyplot as plt
sc.pp.neighbors(new_adata, use_rep="common")
sc.tl.umap(new_adata)
...
all_adata = ad.read_h5ad(wdir+'/model_data/all_lung.h5ad')
...
p = [ x for x in all_adata.var_names if x  in df.columns]
np = [ x for x in all_adata.var_names if x not in df.columns]
df = df[p]
for col in np:df[col] = 0.0
df = df.loc[:,all_adata.var_names]
...
x_z = torch.tensor(new_adata.obsm['common']).float()
y = new_adata.obs_names.values

z,ylabel = model.predict_batch_unique(picasa_unique_model,x_c1,y,x_z)
z_u = z[0]

new_adata.obsm['unique'] = z_u.detach().numpy()

sc.pp.neighbors(new_adata, use_rep="unique")
sc.tl.umap(new_adata)
sc.tl.leiden(new_adata)
sc.pl.umap(new_adata,color=['leiden'] )
plt.savefig('results/unique_umap.png')
