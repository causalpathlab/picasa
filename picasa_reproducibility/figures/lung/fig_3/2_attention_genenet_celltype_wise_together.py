...
def get_top_genes_per_group(main_attn,main_y,top_n):
    ...
    for idx, ct in enumerate(unique_celltypes):
        ...
        top_genes[ct]= [x+'_'+y for x,y in zip(df_attn['level_0'],df_attn['level_1'])]
...
for cell_type, genes in gene_dict.items():
    for gene in genes:
        G.add_node(gene.split('_')[0], cell_type=cell_type)
        G.add_node(gene.split('_')[1], cell_type=cell_type)
...
for node in G.nodes():
    gene_cell_types = []
    for cell_type, gene_pairs in gene_dict.items():
        for gp in gene_pairs:
            if node in gp.split('_'):
                gene_cell_types.append(cell_type)
...
plt.scatter([], [], color='yellowgreen', label='Overlapping Genes')
