...
def integrate_and_label_data(batch_map, label_file):
    ...
    label_df.rename(columns={'index': 'cell'}, inplace=True)
    combined_adata.obs = pd.merge(combined_adata.obs, label_df, left_index=True, right_on='cell', how='left')
    return combined_adata


def run_external_analysis(adata,df, method, save_path, batch_key='Batch', group_key='Group'):
    ...
    if method == 'bbknn':
        ...
        df = pd.read_csv(DATA_DIR+'/simulation/harmony_integrated_PCA.csv')
        df.set_index('Unnamed: 0',inplace=True)
        # sc.pp.pca(adata)
        # sc.external.pp.harmony_integrate(adata, batch_key)
        adata.obsm['X_pca_harmony'] = df.values
...
def run_picasa_analysis(df):
    ...
    df_picasa_res = pd.DataFrame(picasa_res,columns=['picasa_Batch','picasa_Group'])
...
if __name__ == "__main__":
    ...
    df.to_csv(os.path.join(RESULTS_DIR, 'benchmark.csv.gz'),compression='gzip')
