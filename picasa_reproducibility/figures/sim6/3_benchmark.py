...
def run_external_analysis(adata,df, method, save_path, batch_key=constants.BATCH, group_key=constants.GROUP):
...
def run_picasa_analysis(df,batch_key=constants.BATCH, group_key=constants.GROUP):
    ...
    df_picasa_res = pd.DataFrame(picasa_res,columns=['picasa'+'_'+batch_key,'picasa'+'_'+group_key])
