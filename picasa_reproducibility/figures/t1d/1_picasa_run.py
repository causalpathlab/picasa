...
for file_name in file_names:
	print(file_name)
	batch_map[file_name.replace('.h5ad','').replace('t1d_','')] = an.read_h5ad(ddir+file_name)
	batch_count += 1
	if batch_count >=12:
		break


file_name = file_names[0].replace('.h5ad','').replace('t1d_','')
...
params = {'device' : 'cuda',
		'batch_size' : 100,
		'input_dim' : batch_map[file_name.replace('.h5ad','').replace('t1d_','')].X.shape[1],
		'embedding_dim' : 3000,
		'attention_dim' : 15,
		'latent_dim' : 15,
...
input_dim = picasa_object.data.adata_list['Control'].X.shape[1]
enc_layers = [128,15]
