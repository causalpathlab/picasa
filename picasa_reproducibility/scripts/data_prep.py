...
def prep_lung_data():
	...
	adata.write('all_lung.h5ad',compression='gzip')
...
def prep_gbm_data():
	import h5py as hf
	f = hf.File('Glioma_GSE138794_expression.h5')
	...
	dfl = pd.read_csv('Glioma_GSE138794_CellMetainfo_table.tsv',sep='\t')

	# patients more than 1.5k cells
	sel_patient = dfl.Patient.value_counts().index[:6]
	dfl = dfl[dfl['Patient'].isin(sel_patient)]
	...
	adata.write('all_gbm.h5ad',compression='gzip')
