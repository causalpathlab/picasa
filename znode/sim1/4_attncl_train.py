...
def plot_latent():
	...
	for batch in batch_keys:
		...
		umap_2d = umap.UMAP(n_components=2, init='random', random_state=0,min_dist=1.0, n_neighbors=30,metric='cosine').fit(df)
...
def plot_scsp_overlay():
	...
	umap_2d = umap.UMAP(n_components=2, init='random', random_state=0,min_dist=0.3, n_neighbors=15,metric='cosine').fit(dfh)
	...
	dfm = pd.DataFrame([''.join(x.split('_')[1:]) for x in df_umap['cell']],columns=['cell'])
	dflabel= pd.merge(dfm,dfl,on='cell',how='left')
 
	df_umap['celltype'] = dflabel['celltype']
	df_umap['batch'] = dflabel['batch']
