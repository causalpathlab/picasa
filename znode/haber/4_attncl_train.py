...
file_name = file_names[0].replace('.h5ad','').replace('haber_','')
...
params = {'device' : 'cuda',
		'batch_size' : 100,
		'input_dim' : batch_map[file_name.replace('.h5ad','').replace('haber_','')].X.shape[1],
		'embedding_dim' : 1000,
		'attention_dim' : 15,
		'latent_dim' : 15,
		...
		'learning_rate' : 0.001,
		'lambda_loss' : [1.0,0.5,0.0,1.0],
		'temperature_cl' : 1.0,
		'pair_search_method' : 'approx_50',
	 	'corruption_tol' : 10,
		'pair_importance_weight' : 0.001,
...
def train():	
	...
	picasa_object.train()
	picasa_object.plot_loss('common')
