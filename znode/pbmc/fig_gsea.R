...
msigdbr_df <- msigdbr::msigdbr(species = "human", category = "C3",subcategory = "TFT:GTRD")
...
for (ct in cts) {
    file = paste("znode/pbmc/results/sc_context_module_gset_",ct,".csv.gz",sep="")
    df = read.table(file, sep = ",", header=TRUE,row.names=1)

    cutoff = 0.05
    outfile=paste('znode/pbmc/results/sc_context_gsea_',ct,'.pdf',sep='')
    run_gsea(df,cutoff,msigdbr_df,outfile)
