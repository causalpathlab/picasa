...
sc.pl.umap(adata, color=["patient_id","cell_type","treatment_phase"])
...
bbknn.bbknn(adata_bbknn, batch_key='patient_id')
sc.tl.leiden(adata_bbknn,resolution=1.0)
sc.tl.umap(adata_bbknn)
sc.pl.umap(adata_bbknn, color=["patient_id","cell_type","treatment_phase"])
plt.savefig(wdir+'results/scanpy_umap_bbknn.png')


adata_combat = adata.copy()
sc.pp.combat(adata_combat, key='patient_id')
sc.pp.scale(adata_combat)
sc.pp.pca(adata_combat, use_highly_variable=False)
sc.pp.neighbors(adata_combat)
sc.tl.umap(adata_combat)
sc.pl.umap(adata_combat, color=["patient_id","cell_type","treatment_phase"])
plt.savefig(wdir+'results/scanpy_umap_combat.png')


adata_mnn = adata.copy()
adata_mnn_corrected = sc.external.pp.mnn_correct(adata_mnn, batch_key='patient_id')[0][0]
