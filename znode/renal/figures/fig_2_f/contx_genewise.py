...
for idx, ct in enumerate(sel_clust):
	...
	df_cntx = pd.DataFrame(np.mean(p1_context[ct_yindxs], axis=0),
						index=adata_p1.var.index.values)
	df_cntx = df_cntx.unstack().reset_index()
	df_cntx = df_cntx.sort_values(0,ascending=False)
	top_genes.append(df_cntx['level_1'].unique()[:top_n])
...
for idx, ct in enumerate(sel_clust):
	...
	df_cntx = df_cntx.apply(zscore)
	df_cntx.fillna(0.0,inplace=True)
	df_cntx[df_cntx > 5] = 5
	df_cntx[df_cntx < -5] = -5
	# df_cntx = df_cntx.loc[:,top_genes]
	df_cntx = df_cntx.loc[top_genes,:]

	print(ct, df_cntx.shape)
 
	sns.heatmap(df_cntx, cmap='vlag', ax=axes[idx])
...
plt.tight_layout()
plt.savefig(wdir +cdir+ 'sc_context_allct.png')
plt.close()
...
for idx, ct in enumerate(sel_clust):
	...
	df_cntx[df_cntx < -5] = -5
	df_cntx = df_cntx.loc[top_genes,:]
	df_cntx.columns = [ct+'_'+str(x)for x in df_cntx.columns]
	df_all = pd.concat([df_all,df_cntx],axis=1)
	print(df_all.shape)


plt.figure(figsize=(35, 15))
sns.heatmap(df_all, cmap='vlag')#,col_cluster=False)
plt.tight_layout()
plt.savefig(wdir +cdir+ 'sc_context_allct_combine.png')
plt.close()
