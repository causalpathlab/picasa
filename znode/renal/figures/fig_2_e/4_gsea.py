...
file_name = file_names[0].replace('.h5ad','').replace('renal_','')
...
params = {'device': 'cuda', 'batch_size': 64, 'input_dim': 2000, 'embedding_dim': 5000, 'attention_dim': 25, 'latent_dim': 25, 'encoder_layers': [100, 25], 'projection_layers': [25, 25], 'learning_rate': 0.001, 'lambda_loss': [1.0, 0.1, 0.5], 'temperature_cl': 1.0, 'neighbour_method': 'approx_50', 'corruption_tol': 3, 'pair_importance_weight': 0.01, 'rare_ct_mode': True, 'num_clusters': 5, 'rare_group_threshold': 0.1, 'rare_group_weight': 2.0, 'epochs': 1, 'titration': 20}
...
for idx, ct in enumerate(sel_clust):
	...
	## for ranking genes add step values
...
for db in dbs:
	try:
		...
		sns.clustermap(df_result, annot=False, cmap=col_p, cbar_kws={'label': 'NES (Normalized Enrichment Score)'})
